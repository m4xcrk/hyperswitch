...
use crate::{
    core::{
        payments,
        webhooks::{
            self as webhooks_core, create_event_and_trigger_outgoing_webhook,
            get_webhook_detail_by_webhook_endpoint_id, get_webhook_details_for_event_type,
            types::OutgoingWebhookTrackingData,
        },
    },
    db::StorageInterface,
    errors, logger,
...
pub async fn trigger_outgoing_webhook_bulk_workflow(
    ...
    .await?;
    let webhook_details =
        match get_webhook_details_for_event_type(&business_profile, event_type).await {
            Ok(details) => details,
            Err(_) => {
